...
1. **Segmentation**
  * Compute the total surface area (in cm<sup>2</sup>) of papyrus sheets present in all four scrolls combined.
...
Lines: 240. Complete lines: 206. Passing lines: 137. Pass rate: 137 / 206 = **67% (needs to be 90%)**.
...
* **Volume Cartographer** - [EduceLab](https://github.com/educelab/volume-cartographer), [spacegaier fork](https://github.com/spacegaier/volume-cartographer)
  * OME-Zarr
    * Accept as input for segmentation
  * Render from OME-Zarr
    * For live flattened segment preview and/or live ink detection (see below)
  * Multi-axes viewports (XYZ)
    * Similar to [Khartes](https://github.com/KhartesViewer/khartes)
    * Enable 3D inspection of scroll volume
  * Flattened segment preview
    * Similar to [Khartes](https://github.com/KhartesViewer/khartes)
    * Enable live or near-live inspection of fiber continuity
  * Ink detection preview
    * A live or near-live updated ink detection viewport, similar to the flattened segment preview
    * Interchangeable ink detection models
  * Off-axis viewer
    * Allow 3D rotation (not just XYZ) with updating viewports
  * Off-axis segmentation
    * Allow 3D rotation and with the ability to continue segmentation at any angle of rotation
  * Import and Display pointclouds in VC
  * GUI improvements
    * ~~Ability to rotate slice while segmenting - to reduce head tilt~~
      * ~~Show current angle of rotation~~
      * ~~Proposed keybinds: “s + wheel” and “x” to reset/snap to original orientation~~
    * ~~Other Keybinds~~
      * ~~Evenly Space Points = Proposed keybind: “z/y”~~
      * ~~Alternate Evenly Space Points - Proposed keybind "v"~~
      * ~~Alternate angle of rotation: "u/o"~~
      * ~~Alternate reset/snap to original orientation: "i"~~
      * ~~Change slice down/up values: 3,4 = by 5 | 5,6 = by 10 | 7,8 by 50~~
  * Eliminate dot residue
    * Dots that were moved occasionally appear to remain in original location until view is reset
...
Submissions are evaluated monthly, and multiple submissions/awards per month are permitted. The next deadline is 11:59pm Pacific, June 30, 2024! Submit using [this form](https://forms.gle/SXZemPHjEM9rE6Tt7).
